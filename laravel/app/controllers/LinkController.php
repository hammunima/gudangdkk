...
class LinkController extends BaseController
{
    ...
    */
    ...
    public function alokasi_barang()
    {
        ...
        $m['aset'] = DB::table('aset_data')->where('id_ruangan', '')->where('id_puskesmas', sprintf("%03d", Auth::user()->id_puskesmas))->get();
        ...
        $m['bidang'] = DB::table('tbbidang')->get();
        $m['int'] = DB::table('pkm_puskesmas')->where('id', '<>', Auth::user()->id_puskesmas)->get();
        $m['jb'] = DB::table('pkm_inventori')->where('id_puskesmas', Auth::user()->id_puskesmas)->select(DB::raw('distinct(jenis)'))->get();
    ...
    public function masuk_barang()
    {
        ...
        //$m['baru']=$maks;
        //$m['sup'] = DB::table('tbsupplier')->get();
        $m['subaset'] = DB::table('kode_aset')->select(DB::raw('distinct kode_sub_kelompok'), 'sub_kelompok_deskripsi', 'kode_golongan')->get();
        $m['aset'] = DB::table('kode_aset')->select('kode_sub_kelompok', 'kode_sub_sub_kelompok', 'subsub_kelompok_deskripsi')->get();
    ...
    public function aset_masuk()
    {
        ...
        $tabel = Datatable::table()
            ...
            ->setClass('table datatable')
            ->noScript();
        $m['brg'] = DB::table('aset_data')->get();
        $pkm = (int)Auth::user()->id_puskesmas;
        $maks = DB::table('aset_masuk')->where('nomor', 'like', 'AM' . sprintf("%03d", $pkm) . '-' . date('ym') . '%')->max('nomor');
        if ($maks == '') {
            $m['baru'] = 'AM' . sprintf("%03d", $pkm) . '-' . date('ym') . sprintf("%04d", 1);
        } else {
            $m['baru'] = substr($maks, 0, 10) . sprintf("%04d", substr($maks, 10) + 1);
        }
        //$m['baru']=$maks;
        $m['subaset'] = DB::table('kode_aset')->select(DB::raw('distinct kode_sub_kelompok'), 'sub_kelompok_deskripsi', 'kode_golongan')->get();
        $m['aset'] = DB::table('kode_aset')->select('kode_sub_kelompok', 'kode_sub_sub_kelompok', 'subsub_kelompok_deskripsi')->get();
        ...
        $m['bidang'] = DB::table('tbbidang')->get();
        $m['jb'] = DB::table('pkm_inventori')->where('id_puskesmas', Auth::user()->id_puskesmas)->select(DB::raw('distinct(jenis)'))->get();
        return View::make('aset_masuk', compact('tabel'), compact('m'));
    }
    ...
    public function aset_keluar()
    {
        $m = array();
        $tabel = Datatable::table()
            ->addColumn('Nomor', 'Tanggal', 'Tujuan', 'Unit Tujuan', 'Action')
            ->setUrl(route('data-aset-keluar'))
            ->setOptions(
                array(
                    'order' => array(array(0, 'desc'))
                )
        ...
        $m['aset'] = DB::table('aset_data')->where('id_ruangan', '<>', '')->where('id_puskesmas', sprintf("%03d", Auth::user()->id_puskesmas))->get();
        $pkm = (int)Auth::user()->id_puskesmas;
        $maks = DB::table('aset_keluar')->where('nomor', 'like', 'AL' . sprintf("%03d", $pkm) . '-' . date('ym') . '%')->max('nomor');
        if ($maks == '') {
            $m['baru'] = 'AL' . sprintf("%03d", $pkm) . '-' . date('ym') . sprintf("%04d", 1);
        } else {
            $m['baru'] = substr($maks, 0, 10) . sprintf("%04d", substr($maks, 10) + 1);
        }
        $m['unit'] = DB::table('pkm_unit')->get();
        ...
        $m['int'] = DB::table('pkm_puskesmas')->where('id', '<>', Auth::user()->id_puskesmas)->get();
        $m['jb'] = DB::table('pkm_inventori')->where('id_puskesmas', Auth::user()->id_puskesmas)->select(DB::raw('distinct(jenis)'))->get();
        return View::make('aset_keluar', compact('tabel'), compact('m'));
    }
    ...
    public function aset_data()
    {
        $tabel = Datatable::table()
            //->addColumn('Nama Barang', 'Satuan', 'Harga', 'Stok')
            ->addColumn('Kode Aset', 'Nama Aset', 'No Register','Tahun', 'Ruangan', 'Jumlah')
            ->setUrl(route('get-aset-data'))
            ->setClass('table datatable')
            ->noScript();
        //$jenis = DB::table('tbjenis')->get();
        $m['jb'] = DB::table('pkm_inventori')->where('id_puskesmas', Auth::user()->id_puskesmas)->select(DB::raw('distinct(jenis)'))->get();
    ...
    {
        return View::make('entry_aset');
    }

    public function aset_edit()
    {
        $m['brg'] = DB::table('pkm_barang')->get();
        $m['subaset'] = DB::table('kode_aset')->select(DB::raw('distinct kode_sub_kelompok'), 'sub_kelompok_deskripsi', 'kode_golongan')->get();
        $m['aset'] = DB::table('kode_aset')->select('kode_sub_kelompok', 'kode_sub_sub_kelompok', 'subsub_kelompok_deskripsi')->get();
        $m['sup'] = DB::table('pkm_supplier')->where('id_puskesmas', Auth::user()->id_puskesmas)->orWhere('id_puskesmas', '')->get();
        $aset=DB::table('aset_data')->where('id_puskesmas',sprintf("%03d", Auth::user()->id_puskesmas))->where('valid','1')->orderBy('id_aset')->groupBy('id_aset')->get();
        return View::make('aset_edit',compact('aset'),compact('m'));
    }

    public function aset_kib(){
        return View::make('aset_kib');
    }
    ...
    public function aset_kir()
    {
        $m['unit'] = DB::table('pkm_unit')->get();
        $m['bidang'] = DB::table('tbbidang')->get();
        return View::make('aset_kir',compact('m'));
    }
    ...
    {
        $tabel = Datatable::table()
            ->addColumn('Nama Pegawai', 'NIP','Jabatan', 'Unit Kerja', 'Action')
            ->setUrl(route('get-pegawai'))
            ->setClass('table datatable')
            ->noScript();
