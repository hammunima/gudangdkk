    ...
    public function report_stok()
    {
        ...
        if (Input::get('jns') == 1) {
            if (Input::get('p_jns') != 0) {
                ...
                $barang = DB::table('tbbarang')->where('cKdJenis', $j[0])->orderBy('cKdJenis', 'asc')->orderBy('cNama', 'asc')->get();
            } else {
                $barang = DB::table('tbbarang')->orderBy('cKdJenis', 'asc')->orderBy('cNama', 'asc')->get();
            }
            $data = 'LAPORAN DATA STOCK BARANG PER JENIS BARANG PERIODE ' . date('d-m-Y', strtotime($awal)) . ' s/d ' . date('d-m-Y', strtotime($akhir));
        } else {
            if (Input::get('p_jns') != 0) {
                $j = explode('-', Input::get('p_jns'));
                $barang = DB::table('tbbarang')->where('cKdJenis', $j[0])->orderBy('cNama', 'asc')->get();
            } else {
                $barang = DB::table('tbbarang')->orderBy('cNama', 'asc')->get();
            }
