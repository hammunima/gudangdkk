...
@section('content')
    ...
    <div class="page-content-wrap">
        ...
        <div class="row">
            <div class="col-md-12">
                <form class="form-horizontal" method="post" action="{{route('pkm-alokasi')}}">
                    <div class="panel panel-default @if(!$errors->has()) panel-toggled @endif" id="daftar">
                        ...
                        <div class="panel-body" id="bodi">
                            ...
                            <table class="table">
                                ...
                                <tr>
                                    <td><h4>Habis Pakai</h4></td>
                                    <td style="width: 35%;">
                                        <select class="form-control select" data-live-search="true" name="addbrg">
                                            <option value=""></option>
                                            @foreach($m['inventori'] as $row)
                                                <option value="{{$row->id_barang.'-'.$row->nama_barang}}">{{$row->id_barang.' - '.$row->nama_barang}}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td style="width: 35%;">
                                        <select class="form-control select" data-live-search="true" name="addbrg1">
                                            ...
                                            @foreach($m['aset'] as $row)
                                                <option value="{{$row->id_aset.'-'.$row->nama}}">{{$row->id_aset.' - '.$row->nama.' - '.$row->no_register}}</option>
                                            @endforeach
                                        </select>
                                    </td>
        ...
        <!--END MODAL-->
        <!--modal bend-->
        <div class="modal" id="modal_bend" tabindex="-1" role="dialog" aria-labelledby="defModalHead"
             aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                <form class="form-horizontal" method="post" target="_blank" action="{{route('bend')}}">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><span
                                    aria-hidden="true">&times;</span><span
                                    class="sr-only">Close</span></button>
                        <h4 class="modal-title" id="defModalHead">Bend 29</h4>
                    </div>
                    <div class="modal-body">                        
                        <div class="form-group">
                            <input type="text" class="hidden" name="nomor" value="">
                            <label class="col-md-3 col-xs-12 control-label">Pihak Menyerahkan</label>

                            <div class="col-md-6 col-xs-12">
                                <select name="pihak1" class="form-control" id="pihak1"></select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 col-xs-12 control-label">Pihak Menerima</label>

                            <div class="col-md-6 col-xs-12">
                                <select name="pihak2" class="form-control" id="pihak2"></select>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
...
@section('ajax')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#add1').click(function () {
                ...
                if (brg == '') {
                    ...
                    $('#body-table').append('<tr class="success" style="font-size: 10pt">' +
                    '<td><input type="text" class="hidden" name="tipe[]" value="aset"><label class="control-label">ASET</label></td>' +
                    '<td><input type="text" class="hidden" name="barang[]" value="' + brg + '"><label class="control-label">' + brg2 + '</label></td>' +
                    '<td><input type="text" class="hidden" name="satuan[]" value="' + sat + '"><label class="control-label">' + sat + '</label></td>' +
                    '<td><input type="text" class="hidden" name="jumlah[]" value="' + jum + '"><label class="control-label pull-right">' + jum + '</label></td>' +
                    '<td><input type="text" class="hidden" name="keterangan[]" value="' + tot + '"><label class="control-label">' + tot + '</label></td>' +
        ...
        function pop_editt(id) {
            var kode = 'ALO{{sprintf("%03d", Auth::user()->id_puskesmas)}}-' + id;
                ...
                success: function (result) {
                    ...
                    for (i = 0; i < result.dtl.length; i++) {
                        if (result.dtl[i]['tipe'] == 'hp') {
                            ...
                            $('#body-table').append('<tr class="success" style="font-size: 10pt">' +
                            '<td><input type="text" class="hidden" name="tipe[]" value="aset"><label class="control-label">ASET</label></td>' +
                            '<td><input type="text" class="hidden" readonly name="barang[]" value="' + result.dtl[i]['nama'] + '"><label class="control-label">' + result.dtl[i]['nama'] + '</label></td>' +
                            '<td><input type="text" class="hidden" readonly name="satuan[]" value="' + result.dtl[i]['satuan'] + '"><label class="control-label">' + result.dtl[i]['satuan'] + '</label></td>' +
                            '<td><input type="text" class="hidden" readonly name="jumlah[]" value="' + result.dtl[i]['jumlah'] + '"><label class="control-label pull-right">' + result.dtl[i]['jumlah'] + '</label></td>' +
                            '<td><input type="text" class="hidden" readonly name="keterangan[]" value="' + result.dtl[i]['keterangan'] + '"><label class="control-label">' + result.dtl[i]['keterangan'] + '</label></td>' +
                            '<td><a onclick="$(this).parent().parent().remove();" class="btn btn-icon-only red"><i class="fa fa-times"></i></a></td>' +
        ...
        function pop_bend(id) {
            //alert(id);
            $('#detail').empty();
            $.ajax({
                url: '{{route('dkk-get-detail')}}',
                type: 'POST',
                data: {kode: id, tipe: 'bend'},
                dataType: 'json',
                success: function (result) {
                    //alert(result.data.pihak1[0]['nama_pegawai']);
                    $('[name=nomor').val(result.data.nomor);
                    for (i = 0; i < result.data.pihak1.length; i++) {
                        $('#pihak1').append('<option value="'+result.data.pihak1[i]['id']+'">'+result.data.pihak1[i]['nama_pegawai']+'</option>');
                    }
                    for (i = 0; i < result.data.pihak2.length; i++) {
                        $('#pihak2').append('<option value="'+result.data.pihak2[i]['id']+'">'+result.data.pihak2[i]['nama_pegawai']+'</option>');
                    }
                    
                }
            })
            $('#modal_bend').modal('show');
